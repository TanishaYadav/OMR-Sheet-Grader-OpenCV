...
try:
    ...
    img_inverse_transformed = cv2.warpPerspective(img_raw_drawings,inverse_matrix, (img_width,img_height))
    ...
    img_grade_inverse_transformed = cv2.warpPerspective(img_raw_grade,matrix_inverse_grade, (img_width, img_height))
...
# cv2.imshow("Grade result2", img_threshold)
# cv2.imshow("Grade result3", img_raw_drawings)
#
# cv2.imshow("Grade result4", img_inverse_transformed)

#
# if cv2.waitKey(1) & 0xFF == ord('s'):
#     cv2.imwrite("Saved Result" + str(count) + ".jpg", img_final)
# cv2.waitKey(300)
#     # count =  count + 1
k = cv2.waitKey(0)
if k % 256 == 27:
