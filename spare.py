...
try:
    ...
    option_contour = utils.get_corner_points(rect_contours[2])#
    ...
    img_option_contour = img_resized.copy()#
    ...
    if largest_contour.size != 0 and grade_contour.size != 0:
        ...
        cv2.drawContours(img_biggest_contour,option_contour,-1,(255,255,0),30)#
    ...
    option_contour = utils.reorder(option_contour)
    ...
    pt1_option = np.float32(option_contour)
    pt2_option = np.float32([[0, 0], [100, 0], [0, 500], [100, 500]])
    matrix_option = cv2.getPerspectiveTransform(pt1_option, pt2_option)
    img_option_transformed = cv2.warpPerspective(img_resized, matrix_option, (100, 500))
    cv2.imshow('Options' ,img_option_transformed )
    ...
    preprocessed_images_array = ([img_resized, img_gray, img_blur, img_canny],
                                 [img_contours, img_biggest_contour, img_transformed, img_threshold],
                                 [img_result, img_raw_drawings, img_inverse_transformed, img_canvas])
